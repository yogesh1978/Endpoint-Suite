SELECT name,patchname,prvpatch,device_count ,severity,prodfamily,status,type,date_posted,preference,architecture ,acqisition_status,bulletin,prodid,patchuid,vendor,sqnumber,lang_name,languagecode,language
,title,vendor_info_page
FROM  (
SELECT 
  a.patchname ,a.prvpatch,pb.title,pb.vendor_info_page, count(distinct(a.device_id)) as device_count,
...
left join (SELECT patchname,vendor,bulletin,name,
					CASE WHEN rebootpending>0 THEN 'Reboot Pending' 
						...
						SELECT patchname,vendor,bulletin,name,
								 SUM(CASE WHEN LOWER(status)='patchd' THEN 1 ELSE 0 END) AS patched,
...
<<INNERFILTERSTRING>> AND LOWER(a.status)= 'missing'  
...
,a.bulletin,a.prodfamily,a.prodid,a.name,a.patchuid,a.vendor,a.sqnumber,a.language,l.lang_name,l.languagecode) t1 <<OUTERFILTERSTRING>> AND acqisition_status='No'